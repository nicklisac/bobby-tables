...
- **This is the SECOND attempt at Ticket 26.** The first attempt lives on branch `sql-refactor` (commit `cfa67c9`, still on the remote) — preserved for clues, **not** to be re-landed as-is. The next session may `git diff main sql-refactor` to see exactly what was tried (the `utils.js` helpers, the `scratchpad.js`/`chat-render.js` split, the view SQL drafts, the session-savepoint wrappers) and mine it for working code, but every transaction-pattern / migration / vendor-VFS change there is guilty until the 26.1 harness proves it safe.
- The first attempt's uncommitted WIP (BUG-010/011 fixes, close-before-boot, **BUG-012 temporary instrumentation in `vendor/wa-sqlite-jspi/IDBBatchAtomicVFS.js`** gated by `window.__b12off`, BUG_LOG updates, the `bug12-*.html` probes, this re-scope) is in **`stash@{0}`** — recover with `git checkout sql-refactor && git stash pop`. The close-before-boot fix and the one-time gated migration in that stash are the parts worth keeping; the savepoint wrappers and the instrumentation are not.
